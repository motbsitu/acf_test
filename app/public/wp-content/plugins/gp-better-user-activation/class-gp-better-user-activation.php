...
class GP_Better_User_Activation extends GP_Perk {
	...
	public function maybe_redirect_on_success() {
		...
		// redirect user to custom success page
		$redirect_on_success_url = get_permalink( $redirect_on_success_id );
		wp_redirect( $redirect_on_success_url );

	}

	public function hide_title( $title, $post_id ) {
	...
	public function maybe_redirect_activation() {
		...
		$activation_url = get_permalink( $activation_page_id );
		$activation_url .= '?key=' . $_GET['key'];
	...
	public function get_activation_success_redirect_id() {
		...
		return $this->get_setting('activation_success_redirect', $settings);
	}
	...
	public function get_activation_page_id() {
		...
		if( $this->_activation_page_id === null ) {
			$this->_activation_page_id = $this->get_setting( 'activation_page' );
		}
	...
	public function settings() {
		...
		echo self::generate_select( $this, array(
			'id'          => 'activation_success_redirect',
	...
	public function register_settings( $perk ) {
		return array("activation_page", "activation_success_redirect");
	}
